  ...
  const handleMouseMove = (event) => {
    if (isMouseDown) {
      if (event.target.id) {
        ...
        console.log(newRoom);
        setDivGridData(divGridData.map(room =>
             room.id === currentRoomId ? {...room, x:Math.floor((parseInt(event.clientX) - parseInt(initialXPosition)) / 10), y:Math.floor((parseInt(event.clientY) - initialYPosition) / 10)}:room
           ))
      } else {
